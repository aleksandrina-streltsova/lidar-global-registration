...
void estimateOptimalRigidTransformation(const PointNCloud::ConstPtr &src, const PointNCloud::ConstPtr &tgt,
    ...
    for (auto[idx_src, idx_tgt]: inlier_pairs) {
        centroid_src += src->points[idx_src].getVector3fMap();
        centroid_tgt += tgt->points[idx_tgt].getVector3fMap();
    }
    ...
    for (auto[idx_src, idx_tgt]: inlier_pairs) {
        const Eigen::Vector3f &p = src->points[idx_src].getVector3fMap();
        const Eigen::Vector3f &q = tgt->points[idx_tgt].getVector3fMap();
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
