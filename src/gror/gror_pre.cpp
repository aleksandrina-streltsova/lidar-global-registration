...
void GrorPre::grorPreparation(pcl::PointCloud<pcl::PointXYZ>::Ptr origin_cloudS,
                              ...
                              pcl::CorrespondencesPtr corr, pcl::CorrespondencesPtr corr_global, double resolution)
{
    ...
    corr_global->clear();
    for (auto c: *corr) {
        c.index_query = iss_IdxS->indices[c.index_query];
        c.index_match = iss_IdxT->indices[c.index_match];
        corr_global->push_back(c);
    }
