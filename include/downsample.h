...
public:
    void AddPoint(const PointCloudTN::Ptr &pcd, int index) {
    ...
    PointTN GetAveragePoint() const {
        ...
        return PointTN(point_.x / num_of_points_, point_.y / num_of_points_, point_.z / num_of_points_,
                       n.normal_x / norm, n.normal_y / norm, n.normal_z / norm);
    }

private:
    float num_of_points_ = 0;
    PointTN point_ = PointTN(0, 0, 0, 0, 0, 0);
};
